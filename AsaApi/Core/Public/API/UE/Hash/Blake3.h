...
public:
	 ...
	 */
	[[nodiscard]] FBlake3Hash Finalize() const;

	/** Calculate the hash of the buffer. */
	[[nodiscard]] static FBlake3Hash HashBuffer(FMemoryView View);
	[[nodiscard]] static FBlake3Hash HashBuffer(const void* Data, uint64 Size);
	[[nodiscard]] static FBlake3Hash HashBuffer(const FCompositeBuffer& Buffer);
...
[[nodiscard]] FString LexToString(const FBlake3Hash& Hash);
