...
public:
	...
	*/
	...
	static void						SetEnabled(bool InEnable);
	static void						SetFrameTimeThresholdMS(float InTimeMS);
	static void						GetData(TMap<const TCHAR*, FGuardInfo>& Dest);

protected:

	// Used for reporting
	static void						ReportHitch(const TCHAR* InName, float TimeMS, bool VolatileName );
	...
	static float						FrameTimeThresholdMS;
