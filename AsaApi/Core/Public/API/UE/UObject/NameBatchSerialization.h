...
void SaveNameBatch(TArrayView<const FDisplayNameEntryId> Names, TArray<uint8>& OutNameData, TArray<uint8>& OutHashData);

// Save display entries in given order to an archive
void SaveNameBatch(TArrayView<const FDisplayNameEntryId> Names, FArchive& Out);
#endif
